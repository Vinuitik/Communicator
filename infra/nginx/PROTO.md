...
- **Config is COPY-baked, not mounted.** `Dockerfile` does `COPY nginx.conf …` and `COPY static/`. Editing `nginx.conf` or any static file requires **rebuilding the nginx image** (`docker compose build nginx`) — a plain restart runs the old baked copy. This is the #1 "my change didn't take" trap. Applies to every service in the compose file, not just nginx — react-ui hit the same trap 2026-07-30 (connections frontend + group-member-remove sat committed for 5 days while the running image predated both commits).
- **`.git/hooks/post-commit` auto-rebuilds on commit (added 2026-07-30).** Diffs `HEAD`'s changed files against a path→service map (`friend/group/connections/chrono/backup/bootstrap/pom.xml`→`communicator-app`, `react/`→`react-ui`, `nginx/`→`nginx`, etc.) and runs `docker compose build <services> && up -d <services>` in the background so commits aren't blocked; log at `.git/hooks/post-commit-rebuild.log`. **Not versioned** — `.git/hooks/` isn't tracked by git, so this only exists on this machine and won't survive a fresh clone (fine for this app's single-machine LOCAL delivery model, but re-add it manually after a fresh clone).
...
| Auto-rebuild-on-commit path→service map | `.git/hooks/post-commit` (local only, not versioned) |

// Group Knowledge Table Management Module
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Extract group ID from URL - Fixed URL parsing
    ...
    if (pathParts.includes('knowledge')) {
        ...
        if (knowledgeIndex >= 0 && pathParts[knowledgeIndex + 1]) {
            groupId = parseInt(pathParts[knowledgeIndex + 1]);
        }
    }
    ...
    if (!groupId) {
        ...
        if (container) {
            groupId = parseInt(container.getAttribute('data-group-id'));
        }
    }
    
    console.log('Detected group ID:', groupId, 'from URL:', urlPath);
    
    // Table Management Class
    class GroupKnowledgeTableManager {
        constructor() {
        ...
        async loadKnowledgePage(page) {
            if (!groupId) {
                console.log('No group ID available for pagination');
                return;
            }
            
            try {
                console.log(`Loading page ${page} for group ${groupId}`);
                
                // Backend expects 0-based indexing
                const response = await fetch(`${API_BASE_URL}/getKnowledge/${groupId}/page/${page - 1}`);
                if (!response.ok) {
        ...
        createKnowledgeRow(knowledge) {
            ...
            row.innerHTML = `
                ...
                <td data-label="Knowledge" contenteditable="true">${knowledge.text || knowledge.fact || ''}</td>
        ...
        handleFormSubmit(e) {
            ...
            if (fact && importance) {
                ...
                alert('Please enter both knowledge and its importance!');
            }
        }
                ...
                .then(data => {
                    if (data && data.knowledge) {
                        data.knowledge.forEach((knowledge) => {
                            this.addRowToTable(committedTableBody, knowledge.text, knowledge.priority, knowledge.id);
                        });
                    }
        ...
        handleUpdate(row, knowledgeId) {
            const knowledge = row.cells[1].innerText;
            ...
            // Send update request
            fetch(`${API_BASE_URL}/updateKnowledge/${knowledgeId}`, {
                method: 'PUT',
        ...
        addRowToTable(tableBody, fact, importance, id = null) {
            ...
            factCell.setAttribute('data-label', 'Knowledge');
        ...
        // Method for collecting knowledge data from temporary table
        ...
        // Method for sending knowledge data to backend
        sendKnowledgeData(knowledgeData) {
            const id = groupId || this.getIdFromUrl();
    ...
    // Initialize the group knowledge table manager
    const tableManager = new GroupKnowledgeTableManager();

    // Export methods to be used in other modules
    window.GroupKnowledgeTableManager = {
        collectKnowledgeData: () => new GroupKnowledgeTableManager().collectKnowledgeData(),
        sendKnowledgeData: (knowledgeData) => new GroupKnowledgeTableManager().sendKnowledgeData(knowledgeData),
        getIdFromUrl: () => new GroupKnowledgeTableManager().getIdFromUrl()
    };
});
...
export const sendKnowledgeData = (knowledgeData) => {
    ...
    return fetch(`/api/groups/addKnowledge/${id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(knowledgeData)
    })

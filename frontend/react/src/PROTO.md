...
Files: tailwind.config.js, src/styles/globals.css, components/atoms/{Avatar,Logo,ProgressBar}, components/molecules/{Tabs,SegmentedControl,RatingPicker,ConfirmDialog,Toast,FriendPicker}
...
Shared primitives: `Tabs` (Profile hub's tab row), `SegmentedControl` (Structured/Raw toggle, Insights metric/range pills — same active-color for every option), `RatingPicker` (Great/Okay/Bad chips — each option needs its **own** color, so it's a separate component from SegmentedControl; used by `QuickLogModal` and `AddFriendForm`, options array is `EXPERIENCE_RATINGS`), `ConfirmDialog` (replaces `window.confirm`), `Toast` (`useToast()`, replaces `window.alert`/ad-hoc flash messages — `showToast(message, variant?)`, variant is `'success'` (default, green ✓) or `'error'` (red ✕), added Stage 7 when Settings/GroupDetailsPage's failure paths needed something other than a green checkmark), `FriendPicker` (owns the `getShortFriendList()` fetch + loading/error state — added when the offline-PWA work found `CreateConnectionForm` and `MeetingEditModal` each re-implementing that fetch; controlled `value`/`onChange`, discriminated on `multiple`: single id or `number[]`; `variant: 'dropdown' | 'list'` (dropdown = native `<select>`, list = row list, defaults to dropdown for single-select / list for multi-select), `searchable` adds a name-filter input to the list variant — used by `CreateConnectionForm` (two single dropdowns) and `MeetingEditModal` (one multi list); `ShareLandingPage`'s share-to-friend picker (single + `variant="list"` + `searchable`) is the reason the search mode exists, not retrofitted yet elsewhere — see `components/molecules/FriendPicker/FriendPicker.tsx`).
...
`ConnectionsPage` and `GroupDetailsPage` also call `getShortFriendList()` but were deliberately **not** retrofitted onto `FriendPicker`: `ConnectionsPage` uses it as an id→name lookup map (no selection UI at all), and `GroupDetailsPage`'s "add people to group" modal is a multi-select filtered to exclude existing members — a shape `FriendPicker` doesn't cover yet. Revisit if a third consumer needs the same exclusion-filtered multi-select.

...
> **Proto, not a flow.** The user-facing "generate a friend's knowledge summary" and "chat about a friend" pipelines are in [flows/](../flows/). This maps the internals + the many external seams (Gemini, embedder, Postgres/ParadeDB, Redis, MCP, friend).

Files: main.py, routers/chat.py, routers/knowledge.py, services/{agent,knowledge,fact,fact_validation,chunking,embedding,search,friend_api,knowledge_cache,mcp}_service.py, repositories/{postgres,redis,fact}_repository.py, db/schema.sql, prompts/{prompt_manager,summary_prompt_service}.py, config/settings.py (+ config/config.yaml), dependencies/deps.py
...
2. **Knowledge summarization** — turn a friend's raw knowledge notes into **validated, referenced key-value facts** (a RAG pipeline with AI fact-checking), stored in Postgres, cached in Redis.

`depends_on: mcp-knowledge-server, postgres, embedder`. Loads `ai_agent/.env` (secrets) + `config/config.yaml` (everything else). DI is wired in `dependencies/deps.py`; `main.py startup` eagerly builds the agent (fails hard if MCP/LLM won't init). **Mongo retired 2026-07-23** — everything this service persists now lives in Postgres/ParadeDB (same instance the JVM app uses), applied idempotently at startup via `PostgresRepository._apply_schema()` → `db/schema.sql`.
...
| **Postgres/ParadeDB** (`postgresDB:5432`, same DB the JVM app uses) | persist chunks/embeddings/facts/references | `PostgresRepository` (asyncpg + pgvector), `databases.postgres.dsn` |
| **Redis** (`redisCache:6379`) | summary-cached flags + embedding cache | `RedisRepository`, `databases.redis.url` |
| **Google Gemini** (cloud) | summary + fact validation + chat LLM | `ChatGoogleGenerativeAI`, **`GEMINI_API_KEY`** (`ai_agent/.env`), model `llm.model` |

## Gotchas / Technology Notes

- **~~Dual/confused LLM auth~~ FIXED (2026-07-23).** `_setup_llm` used to pass `google_api_key=None` and force `GOOGLE_APPLICATION_CREDENTIALS=/app/service-account-key.json` — a file that never existed on disk, so the container crash-looped (`ValueError: GEMINI_API_KEY environment variable is required` at import, then a pydantic auth error even once the key was set). Now `_setup_llm` passes `google_api_key=settings.gemini_api_key` directly; the service-account env line is removed. Only `ai_agent/.env → GEMINI_API_KEY` is needed. (Also stopped logging the raw key in the startup `print`.)
...
- **Ollama container is still running, deliberately unused by this service.** A separate future conversation (not yet had) will decide whether it comes back for a privacy-motivated local-LLM chat/summarize path — see `host-wrapper` row below. Don't repoint anything at it without revisiting that decision first.
...
| LLM credentials | `ai_agent/.env GEMINI_API_KEY` (must exist) |
...
| Chunk/embedding/fact schema | `ai_agent/db/schema.sql` (applied idempotently at startup, `CREATE ... IF NOT EXISTS` only) |
